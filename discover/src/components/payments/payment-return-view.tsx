...
function ReturnMessage({ title, body, booking }: { title: string; body: string; booking?: BookingRecord }) {
  return <main className="grid min-h-[100dvh] place-items-center bg-[#101011] px-6 text-center text-[#e8e6df]"><div className="max-w-lg"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/40">Payment</p><h1 className="mt-4 text-3xl font-semibold">{title}</h1><p className="mt-4 text-sm leading-6 text-white/60">{body}</p>{booking && <p className="mt-5 text-sm text-white/75">{new Intl.DateTimeFormat("en-GB", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Ho_Chi_Minh" }).format(new Date(booking.session.startsAt))}{booking.payment?.amountVnd ? ` · ${new Intl.NumberFormat("vi-VN").format(booking.payment.amountVnd)}₫` : ""}</p>}<a href="/bookings" className="mt-8 inline-block rounded-xl bg-[#e8e6df] px-5 py-3 text-sm font-semibold text-[#101011]">View bookings</a></div></main>;
}
