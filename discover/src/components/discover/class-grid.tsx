...
function ClassCard({ session }: { session: BookableSession }) {
  const title = session.offering?.title ?? "Class";
  ...
  const href = `/classes/${encodeURIComponent(slug)}`;
    ...
    <Link href={href} className={styles.card}>
      <span className={styles.cardBody}>
        ...
        <span className={styles.cardMeta}>
          ...
          <span>
            <IconMapPin size={14} aria-hidden="true" /> Class
          </span>
        </span>
      ...
      <span className={styles.cardFooter}>
        <span className={styles.cardPrice}>
          ...
          {!isFree && !isUnknownPrice && (
            <span className={styles.cardUnit}>/ participant</span>
          )}
        </span>
...
interface ClassGridProps {
  ...
  limit?: number;
  /** Optional label shown above the grid (e.g. "Classes"). */
  heading?: string;
...
export function ClassGrid({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const raw = await listBookableSessions({ kind: "class" });
        if (!active) return;
        setSessions(sortFutureBookableSessions(raw));
      } catch {
        if (active) setError("Unable to load classes.");
      } finally {
    ...
    <div className={className}>
      ...
      {!loading && !error && visibleSessions.length === 0 && (
        <p className={styles.emptyState}>
          No classes are available right now. Check back soon.
        </p>
      )}
