  ...
  return (
    <main className="tr-profile-page">
      <article className="tr-profile-shell">
        <div className="tr-profile-cover"><img src={profile.cover} alt="" /></div>
          ...
          <header className="tr-profile-summary">
            <div className="tr-profile-person">
              ...
              <div>
                ...
                <p className="tr-profile-bio">{profile.bio.split("\n").map((line, index) => <span key={index}>{line}<br /></span>)}</p>
              </div>
            </div>
            <div className="tr-profile-actions"><button>Follow</button><button>Message</button></div>
          </header>
          ...
          <div className="tr-profile-layout">
            <div>
              ...
              <section className="tr-profile-skills">
                ...
                <div>{profile.skills.map((skill) => <span key={skill}><Camera />{skill}</span>)}</div>
              </section>
              ...
              {tab === "About" && <section className="tr-profile-empty"><h2>About {firstName}</h2><p>{profile.bio}</p></section>}
            </div>

            <aside className="tr-profile-sidebar">
              <section><h2>About {firstName}</h2><p>{profile.bio}</p></section>
              <section>
                <h2>What I offer</h2>
                {offers.map(([title, description]) => <div className="tr-offer" key={title}><Camera /><span><strong>{title}</strong><small>{description}</small></span></div>)}
              </section>
              <section><h2>Achievements</h2><div className="tr-achievement"><Medal />Featured Creator</div><div className="tr-achievement"><Trophy />Top Rated</div></section>
            </aside>
          </div>
