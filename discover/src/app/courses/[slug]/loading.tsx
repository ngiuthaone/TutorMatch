...
export default function CourseLoading() {
  return (
    <div className="tutoria-page-shell tutoria-marketplace-shell flex flex-col bg-black">
      <TopNav />
      <main className="mx-auto grid w-full max-w-[1480px] grid-cols-1 gap-5 px-4 py-6 lg:grid-cols-[minmax(0,1fr)_370px]" aria-busy="true" aria-label="Loading course details">
        <div className="overflow-hidden rounded-xl border border-white/10 bg-white/[0.035]">
          ...
          <div className="grid grid-cols-2 gap-4 border-t border-white/10 p-6 md:grid-cols-4">
            {Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-12 animate-pulse rounded-lg bg-white/[0.06]" />)}
          </div>
        ...
        <aside className="h-[430px] animate-pulse rounded-xl border border-white/10 bg-white/[0.05]" />
      </main>
    </div>
  );
}
