...
const security: FastifyPluginAsync<{ config: AppConfig }> = async (app, { config }) => {
  await app.register(helmet, { ...(config.NODE_ENV === "production" ? {} : { hsts: false }), referrerPolicy: { policy: "no-referrer" } });
  await app.register(cors, {
    origin: (origin, callback) => callback(null, !origin || config.FRONTEND_ORIGINS.includes(origin)),
