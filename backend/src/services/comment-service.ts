...
export function createSupabaseCommentService(url: string, publishableKey: string, _authService: AuthService) {
  ...
  return {
    async create(
      token: string,
      input: { ownerType: "post" | "article"; ownerId: string; body: string; parentId?: string | null },
    ...
    async listPublic(ownerType: "post" | "article", ownerId: string): Promise<PublicCommentListResult> {
      try {
