...
async function signup(role: "student" | "tutor") {
  const email = `dashboard-test-${randomUUID()}@example.test`;
  return signUpConfirmed({
    ...
    email,
    password,
    metadata: { name: `Dashboard Test ${role}`, role },
...
describe.sequential("dashboard routes", () => {
  ...
  it("GET /api/v1/dashboard/overview returns 401 without auth", async () => {
    ...
    const app = createApp({ config, authService, requireAdmin });
  ...
  it("GET /api/v1/dashboard/overview returns 403 with non-admin auth", async () => {
    const { createApp } = await import("../src/app.js");
    const { createSupabaseAuthService } = await import("../src/lib/supabase.js");
    const { parseEnvironment } = await import("../src/config/env.js");
    const config = parseEnvironment(process.env);
