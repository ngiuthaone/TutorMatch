...
async function signup(role: "student" | "tutor") {
  const email = `admin-test-${randomUUID()}@example.test`;
  return signUpConfirmed({
    ...
    email,
    password,
    metadata: { name: `Admin Test ${role}`, role },
...
describe.sequential("admin routes", () => {
  ...
  it("GET /api/v1/admin/audit-log returns 401 without auth", async () => {
    const { createApp } = await import("../src/app.js");
    const { createSupabaseAuthService } = await import("../src/lib/supabase.js");
    const { createSupabaseAdminService } = await import("../src/services/admin-service.js");
    ...
    const adminService = createSupabaseAdminService(config.SUPABASE_URL, config.SUPABASE_SERVICE_ROLE_KEY!);
    const requireAdmin = async () => { throw new Error("not admin"); };
    const app = createApp({ config, authService, adminService, requireAdmin });
  ...
  it("GET /api/v1/admin/audit-log returns 403 with non-admin auth", async () => {
    const { createApp } = await import("../src/app.js");
    const { createSupabaseAuthService } = await import("../src/lib/supabase.js");
    const { createSupabaseAdminService } = await import("../src/services/admin-service.js");
    const { parseEnvironment } = await import("../src/config/env.js");
  ...
  it("GET /api/v1/admin/search/users returns 401 without auth", async () => {
    const { createApp } = await import("../src/app.js");
    const { createSupabaseAuthService } = await import("../src/lib/supabase.js");
    const { createSupabaseAdminService } = await import("../src/services/admin-service.js");
    const { parseEnvironment } = await import("../src/config/env.js");
