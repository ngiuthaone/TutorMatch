...
async function signup(role: "student" | "tutor") {
  const email = `articles-test-${randomUUID()}@example.test`;
  return signUpConfirmed({
    ...
    email,
    password,
    metadata: { name: `Articles Test ${role}`, role },
    trustedTutor: role === "tutor",
  });
}

describe.sequential("article routes", () => {
  ...
  it("GET /api/v1/articles does not require auth", async () => {
    ...
    const articleService = createSupabaseArticleService(config.SUPABASE_URL, config.SUPABASE_PUBLISHABLE_KEY, authService);
    const app = createApp({ config, authService, articleService });
  ...
  it("POST /api/v1/articles returns 401 without auth", async () => {
    const { createApp } = await import("../src/app.js");
    const { createSupabaseAuthService } = await import("../src/lib/supabase.js");
    const { createSupabaseArticleService } = await import("../src/services/article-service.js");
    const { parseEnvironment } = await import("../src/config/env.js");
    ...
    const res = await app.inject({
      method: "POST",
      url: "/api/v1/articles",
      payload: { title: "Test", contentHtml: "<p>Test</p>", contentJson: {} },
    });
    expect(res.statusCode).toBe(401);
    await app.close();
  });

  it("GET /api/v1/articles/mine returns 401 without auth", async () => {
    const { createApp } = await import("../src/app.js");
    const { createSupabaseAuthService } = await import("../src/lib/supabase.js");
    const { createSupabaseArticleService } = await import("../src/services/article-service.js");
    const { parseEnvironment } = await import("../src/config/env.js");
    ...
    const res = await app.inject({ method: "GET", url: "/api/v1/articles/mine" });
    expect(res.statusCode).toBe(401);
    await app.close();
  });

  it("POST /api/v1/articles returns 200 with valid auth (creates draft)", async () => {
    const { createApp } = await import("../src/app.js");
    const { createSupabaseAuthService } = await import("../src/lib/supabase.js");
    const { createSupabaseArticleService } = await import("../src/services/article-service.js");
    const { parseEnvironment } = await import("../src/config/env.js");
    ...
    const res = await app.inject({
      ...
      payload: { title: "Test Article", contentHtml: "<p>Test content</p>", contentJson: {} },
    });
    expect(res.statusCode).toBe(200);
