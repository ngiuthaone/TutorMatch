...
async function main() {
  ...
  const idxA = parseInt(process.env.BOOKING_A_INDEX ?? await promptLine("  Booking A index: "), 10);
  if (isNaN(idxA) || idxA < 0 || idxA >= eligible.length) {
    console.error("  Invalid index for Booking A.");
    process.exit(1);
  }
  const idxB = parseInt(process.env.BOOKING_B_INDEX ?? await promptLine("  Booking B index (must differ from A): "), 10);
  ...
  const { redirectUrl: redirectUrlA, merchantReference: merchantRefA, amountVnd: amountA } = startResA.json.payment ?? startResA.json;
  ...
  const paymentDataA = readResA.json?.payment ?? readResA.json;
  const paymentStatusA = paymentDataA?.status;
  const bookingStatusA = paymentDataA?.booking?.status;
  ...
  console.log(`    booking.status: ${paymentDataA?.booking?.status ?? paymentDataA?.status ?? "unknown"}`);
  console.log(`    payment.status: ${paymentDataA?.paymentStatus ?? paymentDataA?.status ?? "unknown"}`);
  console.log(`    payment.amountVnd: ${paymentDataA?.amountVnd ?? "unknown"}`);
  ...
  const { redirectUrl: redirectUrlB, merchantReference: merchantRefB, amountVnd: amountB } = startResB.json.payment ?? startResB.json;
