...
describe("environment", () => {
  it("parses valid development defaults", () => { const result = parseEnvironment(valid); expect(result.PORT).toBe(4000); expect(result.TRUST_PROXY).toBe(false); expect(result.VNPAY_REQUEST_TIMEOUT_MS).toBe(15_000); });
  it.each([
