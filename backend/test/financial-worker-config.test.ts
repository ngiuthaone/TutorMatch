...
const valid = {
  NODE_ENV: "development", TUTORIA_ENVIRONMENT: "development", FRONTEND_ORIGINS: "http://localhost:4173", SUPABASE_URL: "https://p.supabase.co",
...
describe("financial worker configuration", () => {
  ...
  it("fails closed when authority or provider configuration is incomplete", () => {
    ...
    expect(() => requireFinancialWorkerConfig(parseEnvironment({ ...valid, NODE_ENV: "production", FRONTEND_ORIGINS: "https://app.example.test" }))).toThrow("TUTORIA_ENVIRONMENT=staging or production");
    expect(() => requireFinancialWorkerConfig(parseEnvironment({ ...valid, NODE_ENV: "production", TUTORIA_ENVIRONMENT: "production", VNPAY_ENVIRONMENT: "sandbox", SUPABASE_URL: "https://p.supabase.co", FRONTEND_ORIGINS: "https://app.example.test" }))).toThrow("VNPAY_ENVIRONMENT=production");
  });
});
