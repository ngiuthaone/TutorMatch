-- Core 1:1 application/read-model boundary. Existing lifecycle RPCs remain the
-- authority; these functions only compose safe product state for the API.
...
create or replace function public.list_bookable_sessions(p_tutor_profile_id uuid default null) returns jsonb
language sql stable security definer set search_path='' as $$
...
join public.tutor_profiles tp on tp.user_id = s.host_id and tp.publication_status = 'published'
where s.status = 'scheduled'
  and (p_tutor_profile_id is null or tp.id = p_tutor_profile_id)
$$;
revoke all on function public.list_bookable_sessions(uuid) from public, anon, authenticated;
grant execute on function public.list_bookable_sessions(uuid) to anon, authenticated;
