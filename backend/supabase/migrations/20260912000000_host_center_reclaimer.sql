-- Host Center reclaimer: tables, RLS, and RPCs.
--
-- This migration replaces the disabled 20260912000000_host_center_tables_and_rpcs
-- migration. That file was moved to _disabled/ because a partial/probe application
-- on production had already created some host-center tables with schema drift
-- (specifically team_members was missing revoked_at, RLS was not enabled on
-- check_in_tokens/check_in_logs, and no RLS policies existed).
--
-- Every object here is written defensively so it is safe to run against BOTH:
--   * a fresh local database (nothing host-center exists yet) — the migration
--     creates all tables, indexes, policies, and functions from scratch; and
--   * production (some tables/functions already exist, some drifted) — CREATE
--     TABLE IF NOT EXISTS / ADD COLUMN IF NOT EXISTS / guarded policy creation /
--     CREATE OR REPLACE FUNCTION reconcile only what is missing.
--
-- The RLS policies use fixed names created inside DO blocks that check
-- pg_policy first, so re-running never fails with "already exists".
set search_path = '';
...
alter table public.check_in_tokens add column if not exists session_id uuid;
alter table public.check_in_tokens add column if not exists token text;
alter table public.check_in_tokens add column if not exists is_used boolean not null default false;
alter table public.check_in_tokens add column if not exists issued_at timestamptz not null default now();
alter table public.check_in_tokens add column if not exists used_at timestamptz;
...
begin
  if not exists (select 1 from pg_policy p join pg_class c on c.oid = p.polrelid
                 where c.relname = 'check_in_tokens' and p.polname = 'check_in_tokens_auth_insert') then
    create policy check_in_tokens_auth_insert on public.check_in_tokens
      for insert to authenticated with check (true);
  end if;
  if not exists (select 1 from pg_policy p join pg_class c on c.oid = p.polrelid
                 where c.relname = 'check_in_tokens' and p.polname = 'check_in_tokens_auth_select') then
    create policy check_in_tokens_auth_select on public.check_in_tokens
      for select to authenticated using (true);
  end if;
  if not exists (select 1 from pg_policy p join pg_class c on c.oid = p.polrelid
                 where c.relname = 'check_in_tokens' and p.polname = 'check_in_tokens_auth_update') then
...
-- ─────────────────────────────────────────────────────────────────────
-- 2. check_in_logs
-- Immutable audit log of check-in activity.
-- Production may already carry a slightly different shape (learner_id/
-- notes columns, nullable token). We reconcile the base columns the RPCs
-- depend on, enable RLS, and add only the check-in policy. Extra legacy
-- columns are left in place to avoid destroying data the probe created.
-- ─────────────────────────────────────────────────────────────────────
...
alter table public.check_in_logs add column if not exists session_id uuid;
alter table public.check_in_logs add column if not exists host_id uuid;
alter table public.check_in_logs add column if not exists action text;
alter table public.check_in_logs add column if not exists token text;
alter table public.check_in_logs add column if not exists performed_at timestamptz not null default now();
...
begin
  if not exists (select 1 from pg_policy p join pg_class c on c.oid = p.polrelid
                 where c.relname = 'check_in_logs' and p.polname = 'check_in_logs_auth_all') then
...
alter table public.promotion_codes add column if not exists host_id uuid;
alter table public.promotion_codes add column if not exists offering_id uuid;
alter table public.promotion_codes add column if not exists code text;
alter table public.promotion_codes add column if not exists discount_type text;
alter table public.promotion_codes add column if not exists discount_value numeric;
alter table public.promotion_codes add column if not exists max_uses int;
alter table public.promotion_codes add column if not exists used_count int not null default 0;
alter table public.promotion_codes add column if not exists starts_at timestamptz;
alter table public.promotion_codes add column if not exists ends_at timestamptz;
alter table public.promotion_codes add column if not exists is_active boolean not null default true;
alter table public.promotion_codes add column if not exists created_at timestamptz not null default now();
...
begin
  if not exists (select 1 from pg_policy p join pg_class c on c.oid = p.polrelid
                 where c.relname = 'promotion_codes' and p.polname = 'promotion_codes_auth_all') then
...
-- Production had a stale copy WITHOUT revoked_at; ADD COLUMN IF NOT EXISTS
-- reconciles it. This column is required by the RLS policy below and by
-- list_host_team.
-- ─────────────────────────────────────────────────────────────────────
...
alter table public.team_members add column if not exists offering_id uuid;
...
alter table public.team_members add column if not exists capability text;
alter table public.team_members add column if not exists invited_by uuid;
alter table public.team_members add column if not exists joined_at timestamptz not null default now();
alter table public.team_members add column if not exists revoked_at timestamptz;
...
begin
  if not exists (select 1 from pg_policy p join pg_class c on c.oid = p.polrelid
                 where c.relname = 'team_members' and p.polname = 'team_members_auth_all') then
...
-- Tracks failed payout attempts. Created if not present (may not exist on
-- prod yet because the disabled migration never reached it).
-- ─────────────────────────────────────────────────────────────────────
...
alter table public.payout_failure_logs add column if not exists host_id uuid;
alter table public.payout_failure_logs add column if not exists payout_period text;
alter table public.payout_failure_logs add column if not exists amount_vnd bigint;
alter table public.payout_failure_logs add column if not exists failure_reason text;
alter table public.payout_failure_logs add column if not exists attempt_count int not null default 0;
alter table public.payout_failure_logs add column if not exists max_attempts int not null default 3;
alter table public.payout_failure_logs add column if not exists status text;
alter table public.payout_failure_logs add column if not exists last_attempt_at timestamptz;
alter table public.payout_failure_logs add column if not exists resolved_at timestamptz;
alter table public.payout_failure_logs add column if not exists resolved_by uuid;
alter table public.payout_failure_logs add column if not exists resolution_note text;
alter table public.payout_failure_logs add column if not exists created_at timestamptz not null default now();
...
begin
  if not exists (select 1 from pg_policy p join pg_class c on c.oid = p.polrelid
                 where c.relname = 'payout_failure_logs' and p.polname = 'hosts_read_own_payout_failures') then
  ...
  if not exists (select 1 from pg_policy p join pg_class c on c.oid = p.polrelid
                 where c.relname = 'payout_failure_logs' and p.polname = 'service_insert_payout_failures') then
    create policy "service_insert_payout_failures" on public.payout_failure_logs
      for insert with check (true);
  end if;
  if not exists (select 1 from pg_policy p join pg_class c on c.oid = p.polrelid
                 where c.relname = 'payout_failure_logs' and p.polname = 'service_update_payout_failures') then
