...
Booking expiry (implemented Option A, migration `20260907000000_booking_expiry_sweep.sql`): the `expire_stale_bookings` worker auto-cancels stale `requested` bookings older than 24h **only when they have no `succeeded` or `pending` payment** (payment absent or `failed`). It mirrors the proven workshop-worker skeleton (`for update skip locked` + version CAS + direct `requested -> cancelled` with `cancelled_by='system'`, `cancelled_reason='auto_expired'`, booking_history append, `BOOKING_CANCELLED` outbox event) and runs as `service_role`-only. Rationale: stale `requested` bookings hard-reserve Session capacity indefinitely; expiring them releases capacity. `pending` payments are owned by the workshop TTL worker; `succeeded` on an unconfirmed booking would create a refund obligation. **Open PRODUCT_DECISION_REQUIRED**: auto-expiry of PAID-but-unconfirmed stale `requested` bookings and any refund obligation for them — deliberately NOT implemented; do not silently extend this worker to paid bookings without a refund-eligibility design.
